...
async function inspectResponsiveGrid(window, viewport) {
  ...
  for (const size of sizes) {
    ...
    if (size.width === 480) {
      ...
      await waitFor(window, 'Boolean(document.querySelector("[data-testid=project-filter-popover]"))', viewport.label + ' filtros abertos em 480px')
      const filterBounds = await evaluate(window, '(() => { const rect = document.querySelector("[data-testid=project-filter-popover]")?.getBoundingClientRect(); return rect ? { left: rect.left, right: rect.right, width: rect.width } : null })()')
      assert(filterBounds && filterBounds.left >= 60 && filterBounds.right <= size.width, 'project filters overflow at 480px: ' + JSON.stringify(filterBounds))
      await screenshot(window, 'projects-responsive-480-filters')
      await key(window, 'Escape')
      await waitFor(window, '!document.querySelector("[data-testid=project-filter-popover]")', viewport.label + ' fechar filtros em 480px')

      await evaluate(window, 'Array.from(document.querySelectorAll("button")).find((button) => button.getAttribute("aria-label")?.includes("lista"))?.click()')
      await waitFor(window, 'Boolean(document.querySelector("[data-testid=project-list]"))', viewport.label + ' lista em 480px')
      const listMenuReady = await evaluate(window, '(() => { const wrap = document.querySelector("[data-testid=project-list] .project-tile-menu-wrap"); const trigger = wrap?.querySelector("button"); if (!trigger || getComputedStyle(wrap).display === "none") return false; trigger.focus(); return document.activeElement === trigger })()')
      assert(listMenuReady, 'list actions are unavailable to keyboard at 480px')
      await evaluate(window, 'document.activeElement?.click()')
      await waitFor(window, 'Boolean(document.querySelector("[data-testid=project-list] .project-action-menu"))', viewport.label + ' menu da lista por teclado')
      window.webContents.sendInputEvent({ type: 'keyDown', keyCode: 'TAB' })
      window.webContents.sendInputEvent({ type: 'keyUp', keyCode: 'TAB' })
      await waitFor(window, 'Boolean(document.activeElement?.closest(".project-action-menu"))', viewport.label + ' navegar pelo menu da lista por teclado')
      await key(window, 'Escape')
      await waitFor(window, '!document.querySelector("[data-testid=project-list] .project-action-menu")', viewport.label + ' fechar menu da lista por Escape')

      await setInputValue(window, '#project-search', 'Fixture 03')
      await waitFor(window, 'document.querySelectorAll("[data-testid=project-list] .project-row").length === 1', viewport.label + ' projeto único para grupo')
      await evaluate(window, 'document.querySelector("[data-testid=project-filter-toggle]")?.click()')
      await evaluate(window, 'document.querySelector(".project-filter-check input")?.click()')
      await waitFor(window, 'document.querySelectorAll(".project-group").length === 1 && document.querySelectorAll(".project-group .project-row").length === 1', viewport.label + ' grupo com um projeto')
      await evaluate(window, 'document.querySelector(".project-group .project-tile-menu-wrap button")?.click()')
      await waitFor(window, 'Boolean(document.querySelector(".project-group .project-action-menu"))', viewport.label + ' menu do grupo')
      const groupMenu = await evaluate(window, '(() => { const group = document.querySelector(".project-group"); const menu = group?.querySelector(".project-action-menu"); return { overflow: group ? getComputedStyle(group).overflow : "", menuHeight: menu?.getBoundingClientRect().height || 0, actions: menu?.querySelectorAll("button").length || 0 } })()')
      assert(groupMenu.overflow === 'visible' && groupMenu.menuHeight > 0 && groupMenu.actions > 0, 'grouped project actions were clipped: ' + JSON.stringify(groupMenu))
      await evaluate(window, '(() => { const button = document.querySelector("[data-testid=project-filter-toggle]"); if (button?.getAttribute("aria-expanded") !== "true") button?.click(); return true })()')
      await waitFor(window, 'Boolean(document.querySelector("[data-testid=project-filter-popover]"))', viewport.label + ' reabrir filtros agrupados')
      const groupingCleared = await evaluate(window, '(() => { const input = document.querySelector(".project-filter-check input"); if (!input) return false; if (input.checked) input.click(); return !input.checked })()')
      assert(groupingCleared, 'could not clear grouped project filter')
      await waitFor(window, 'Boolean(document.querySelector("[data-testid=project-list]"))', viewport.label + ' remover agrupamento')
      await setInputValue(window, '#project-search', '')
      await waitFor(window, 'document.querySelectorAll("[data-testid=project-list] .project-row").length >= 30', viewport.label + ' limpar busca responsiva')
      await evaluate(window, 'Array.from(document.querySelectorAll("button")).find((button) => button.getAttribute("aria-label")?.includes("grade"))?.click()')
      await waitFor(window, 'Boolean(document.querySelector("[data-testid=project-grid]"))', viewport.label + ' retornar à grade em 480px')
      recordPass(viewport.label, 'filtros, lista por teclado e menus agrupados sem recorte em 480px')
    }
  }
