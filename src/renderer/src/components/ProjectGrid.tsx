...
export const ProjectGrid: React.FC<ProjectGridProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      {/* Filter Tabs Bar */}
      <div className="px-6 py-3 border-b border-slate-800/60 bg-[#090d16]/60 backdrop-blur-sm flex flex-wrap items-center justify-between gap-3">
        {/* Main Status Tabs */}
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto py-1">
            ...
            {selectedTech && (
              <button
                ...
                className="text-[11px] px-2 py-0.5 rounded-full bg-slate-800 text-slate-300 hover:text-white border border-slate-700"
              >
                Limpar
              </button>
            )}
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {filteredProjects.length === 0 ? (
          <div className="h-64 flex flex-col items-center justify-center text-slate-500 border border-dashed border-slate-800 rounded-2xl">
            <FolderX className="w-10 h-10 text-slate-600 mb-3" />
