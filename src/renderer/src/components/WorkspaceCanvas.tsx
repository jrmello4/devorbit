...
export const WorkspaceCanvas: React.FC<{ project: Project; workbench: React.ReactNode; browser: React.ReactNode }> = ({ project, workbench, browser }) => {
  ...
  useEffect(() => {
    if (!drag) return
    const move = (event: PointerEvent) => setState((current) => ({ ...current, cards: current.cards.map((card) => card.id !== drag.id ? card : { ...card, x: Math.max(8, drag.card.x + event.clientX - drag.sx), y: Math.max(8, drag.card.y + event.clientY - drag.sy), z: Math.max(...current.cards.map((item) => item.z)) + 1 }) }))
  ...
  useEffect(() => {
    if (!resize) return
    const move = (event: PointerEvent) => setState((current) => ({ ...current, cards: current.cards.map((card) => card.id !== resize.id ? card : { ...card, width: Math.max(220, Math.min(980, resize.card.width + event.clientX - resize.sx)), height: Math.max(150, Math.min(760, resize.card.height + event.clientY - resize.sy)), z: Math.max(...current.cards.map((item) => item.z)) + 1 }) }))
